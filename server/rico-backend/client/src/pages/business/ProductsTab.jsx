...
export default function ProductsTab({ authedFetch, activeClaims, activeBusinessId, onChangeBusiness }) {
  ...
  async function handleSave(e) {
    ...
    const res = editingId
      ? await authedFetch(`/business/products/${editingId}`, { method: 'PATCH', body: JSON.stringify(payload) })
      : await authedFetch('/business/products', { method: 'POST', body: JSON.stringify({ ...payload, businessId: activeBusinessId }) });
    setSaving(false);
    if (!res || !res.ok) {
  ...
  async function handleDelete(id) {
    ...
    const res = await authedFetch(`/business/products/${id}`, { method: 'DELETE' });
    if (res && res.ok) load();
  }
